...
public class  signLanguageClass {
    ...
    signLanguageClass(Context context, Button clear_button, Button add_button, TextView change_text, Button text_speech_button, Button backspace_button, AssetManager assetManager, String modelPath, String labelPath, int inputSize, String classification_model, int classification_input_size) throws IOException{
        ...
        backspace_button.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                String word = change_text.getText().toString();
                int input = word.length();
                if (input > 0){
                    final_text=word.substring(0, input-1);
                    change_text.setText(final_text);
                }
            }
        });
    ...
    public Mat recognizeImage(Mat mat_image){
        ...
        for (int i=0;i<10;i++){
            ...
            if(score_value>0.5){
                ...
                System.out.println("Output: "+ Arrays.deepToString(output_class_value));
